...
Device client.save_lwm2m_device_with_credentials(request_body: Dict[str, object])  # Save LwM2M device with credentials (saveLwm2mDeviceWithCredentials) (Deprecated)
```
...
```python
Device client.save_lwm2m_device_with_credentials(request_body: Dict[str, object])
```
