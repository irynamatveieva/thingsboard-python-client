...
class TotpTwoFaAccountConfig(TwoFaAccountConfig):
    ...
    def auth_url_validate_regular_expression(cls, value):
        ...
        if not re.match(r"otpauth:\/\/totp\/(\S+?):(\S+?)\?issuer=(\S+?)&secret=(\w+?)", value):
            raise ValueError(r"must validate the regular expression /otpauth:\/\/totp\/(\S+?):(\S+?)\?issuer=(\S+?)&secret=(\w+?)/")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of TotpTwoFaAccountConfig from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of TotpTwoFaAccountConfig from a dict"""
        if obj is None:
            return None
