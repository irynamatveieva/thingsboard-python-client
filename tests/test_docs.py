...
def test_ce_docs_exist_outside_package():
    ...
    assert len(md_files) >= 700, f"ce/docs/ has {len(md_files)} .md files (expected >=700)"
...
def test_ce_model_docs_exist():
    ...
    assert model_count >= 600, f"ce/docs/ has {model_count} model .md files (expected >=600)"
...
def test_ce_model_doc_has_python_conventions():
    ...
    content = doc.read_text(encoding="utf-8")
    assert "model_dump" in content, "Device.md missing Pydantic convention 'model_dump'"
...
def test_pe_docs_exist():
    ...
    assert len(md_files) >= 900, f"pe/docs/ has {len(md_files)} .md files (expected >=900)"
    assert len(api_files) >= 78, f"pe/docs/ has {len(api_files)} *Api.md files (expected >=78)"
...
def test_pe_model_doc_has_correct_package():
    ...
    for doc in model_docs[:20]:
        ...
        if "tb_pe_client.models" in content:
            found = True
            break
    assert found, "No PE model doc contains 'tb_pe_client.models' package reference"
    # Also assert no PE doc references tb_ce_client
    ce_refs = [f for f in model_docs[:20] if "tb_ce_client" in f.read_text(encoding="utf-8")]
    assert not ce_refs, f"PE model docs contain 'tb_ce_client' reference: {ce_refs[0].name}"
...
def test_paas_docs_exist():
    ...
    assert len(md_files) >= 900, f"paas/docs/ has {len(md_files)} .md files (expected >=900)"
    assert len(api_files) >= 83, f"paas/docs/ has {len(api_files)} *Api.md files (expected >=83)"
...
def test_paas_model_doc_has_correct_package():
    ...
    for doc in model_docs[:20]:
        ...
        if "tb_paas_client.models" in content:
            found = True
            break
    assert found, "No PaaS model doc contains 'tb_paas_client.models' package reference"
    ce_refs = [f for f in model_docs[:20] if "tb_ce_client" in f.read_text(encoding="utf-8")]
    assert not ce_refs, f"PaaS model docs contain 'tb_ce_client' reference: {ce_refs[0].name}"
...
def test_ce_wheel_excludes_docs():
    ...
    assert not docs_entries, f"CE wheel contains docs/ entries: {docs_entries[:5]}"
