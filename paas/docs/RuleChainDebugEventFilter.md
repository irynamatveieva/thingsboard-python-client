...
`tb_paas_client.models.RuleChainDebugEventFilter`
...
#### StatisticsEventFilter  *(extends EventFilter, event_type=`STATS`)*
| Name | Type | Description | Notes |
|------|------|-------------|-------|
| server | str | String value representing the server name, identifier or ip address where the platform is running | [optional] |
...
- **Deserialize:** `RuleChainDebugEventFilter.model_validate(data)` or `RuleChainDebugEventFilter.model_validate_json(json_str)`
