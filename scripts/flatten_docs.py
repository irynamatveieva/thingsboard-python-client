...
chase ``[Type](Type.md)`` links to understand the structure.
...
# Matches a complex-type link in a properties-table cell:
#   [**DeviceData**](DeviceData.md)   or   [**List[Foo]**](Foo.md)
_COMPLEX_RE = re.compile(r"\[\*\*(.+?)\*\*\]\((\w+)\.md\)")

# Matches a properties-table row
_PROP_ROW_RE = re.compile(r"\| \*\*(\w+)\*\* \| (.+?) \| (.*?) \|(.*?)\|")


def parse_model_doc(path: Path) -> dict:
    ...
    for m in _PROP_ROW_RE.finditer(content):
        ...
        cm = _COMPLEX_RE.search(type_cell)
        if cm:
            display_type = cm.group(1)
            complex_type = cm.group(2)
            _add_ref(complex_type)
        else:
            display_type = type_cell.replace("**", "")
            complex_type = None
...
def collect_types_to_expand(
    ...
    while queue:
        ...
        parsed = parse_model_doc(doc_path)
        ...
        if max_depth == 0 or depth < max_depth:
            ...
            if (
                discriminators
                and type_name in discriminators
...
def load_discriminators(spec_path: Path, docs_dir: Path) -> dict:
    ...
    renames (e.g. schema ``None`` → class ``ModelNone``) are handled.
    """
    ...
    for name, schema in schemas.items():
        ...
        for value, ref in disc["mapping"].items():
            ...
            # The generator prefixes "Model" to names that clash with
            # Python builtins (e.g. None → ModelNone).
...
def _camel_to_snake(name: str) -> str:
    """Convert camelCase to snake_case (mechanical fallback)."""
    s = re.sub(r"([A-Z]+)([A-Z][a-z])", r"\1_\2", name)
    s = re.sub(r"([a-z0-9])([A-Z])", r"\1_\2", s)
    return s.lower()


def _resolve_prop_name(camel_name: str, properties: list[dict]) -> str:
    """Find the snake_case property name that the generator produced for
    the given camelCase *camel_name*.

    Resolution order:
      1. Exact match against doc properties (handles ``type``, ``spec``, etc.)
      2. Fuzzy match: strip underscores and compare lowercase
      3. Mechanical camelCase→snake_case (when the property is missing from
         the doc entirely, e.g. a discriminator not listed in ``properties``)
    """
    # Exact match (already snake_case or single-word like "type")
    for p in properties:
        if p["name"] == camel_name:
            return camel_name
    # Match by lowercasing both sides (covers type/Type edge-cases)
    camel_lower = camel_name.lower()
    for p in properties:
        if p["name"].replace("_", "") == camel_lower:
            return p["name"]
...
def build_subtypes_section(
    ...
    """
    prop = _resolve_prop_name(disc_info["property"], properties)
    ...
    for value in sorted(mapping):
        ...
        sub_info = parse_model_doc(doc_path)
...
def _fmt_enum(values: list) -> str:
    ...
    return " | ".join(f"`{v}`" for v in shown) + f" | … ({len(values)} values total)"
...
def _append_props_table(
    ...
    inherited_from_eid = info["parent"] == "EntityId" or info.get("parent") in entity_id_subtypes
    lines.append("| Name | Type | Description | Notes |")
    lines.append("|------|------|-------------|-------|")
    if inherited_from_eid:
        lines.append("| entity_type | str | *(inherited)* | |")
        lines.append("| id | UUID | *(inherited)* | |")
    for prop in info["properties"]:
        lines.append(
            f"| {prop['name']} | {prop['type']} | {prop['description']} | {prop['notes']} |"
        )
...
def _append_type_block(
    ...
    *subtype_disc* maps ``subtype_name → (parent, value, prop)``
    ...
    inherited_from_eid = (
        info.get("parent") == "EntityId" or info.get("parent") in entity_id_subtypes
    )
    ...
    if eid_refs:
        ...
        if names:
            ...
            lines.append(
                ...
                + "`{entity_type: EntityType, id: UUID}` — "
                + "all EntityId subtypes share this structure."
            )
        else:
            lines.append(
                "> **EntityId**: `{entity_type: EntityType, id: UUID}` — "
    ...
    if discriminators:
        for parent_name, disc in discriminators.items():
            if parent_name not in types:
                continue
            prop = _resolve_prop_name(
                disc["property"],
                types[parent_name]["properties"],
            )
...
def flatten_doc(
    ...
    # Skip API docs, examples, etc.
    if doc_path.name.endswith("Api.md"):
        return False

    info = parse_model_doc(doc_path)
    ...
    if _CONVENTIONS_MARKER in content:
        content = content.replace(_CONVENTIONS_MARKER, combined + _CONVENTIONS_MARKER)
    else:
...
def main():
    ...
    for md_file in sorted(docs_dir.glob("*.md")):
        if md_file.name in SKIP_FILES or md_file.name.endswith("Api.md"):
            continue
        total += 1
        if flatten_doc(md_file, docs_dir, entity_id_subtypes, max_depth, discriminators):
            count += 1
